...
function renderNav(string $active = 'vault'): string {
    ...
    $adminClass = $active === 'admin'     ? 'active' : '';
    $adminLink  = !empty($_SESSION['is_admin'])
        ? '<a href="admin_dashboard.php" class="' . $adminClass . '">⚙️ Admin</a>'
        : '';
...
function renderAdminNav(string $active = 'dashboard'): string {
    $tabs = [
        'dashboard'  => ['url' => 'admin_dashboard.php',  'icon' => '📊', 'label' => 'Overview'],
        'users'      => ['url' => 'admin_users.php',      'icon' => '👥', 'label' => 'Users'],
        'audit'      => ['url' => 'admin_audit.php',      'icon' => '📋', 'label' => 'Audit Log'],
        'categories' => ['url' => 'admin_categories.php', 'icon' => '🏷️', 'label' => 'Categories'],
    ];
    ...
    foreach ($tabs as $key => $tab) {
        ...
        $nav .= '<a href="' . $tab['url'] . '" class="' . $cls . '">' . $tab['icon'] . ' ' . $tab['label'] . '</a>';
    }
    $nav .= '<a href="vault.php">🔑 Vault</a>'
          . '<a href="logout.php" class="danger">🔒 Lock</a>'
          . '</div></nav>';
    return $nav;
}
